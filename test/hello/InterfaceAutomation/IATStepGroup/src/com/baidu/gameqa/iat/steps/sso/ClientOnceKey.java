...
/**
 * 客户端加密登录获取一次性Key接口
 * API:/sso/client/security/onceKey.json
 * 
 * @author yanglei12
 * @version V1.0 创建时间：2014年5月26日 下午5:41:14
 */
public class ClientOnceKey {
	 ...
	 * @throws Exception
	 * @author yanglei12
	 * @date 2014年5月26日 下午6:27:27
	 */
	@StepMethodDesc(description = "get once key api method", owner = "lei.yanglei")
	public static String getOnceKey(String parameterID) throws Exception {
		String random = testParameters.getValue("random");
		String onceKeySign = SignVerfiy.getSign(bodyMap.get("userId"),
				bodyMap.get("loginToken"), random);
		...
		urlParameters.put("random", random);
		urlParameters.put("sign", onceKeySign);

		actualResult = WebConversationUtils.getWebResponse(
				testParameters.getValue("oncekey_url"),
				testParameters.getValue("onceKey_httpmethod"), urlParameters)
				.getText();
		return actualResult;
	}
